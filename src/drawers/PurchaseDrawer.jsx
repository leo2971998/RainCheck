...
import PurchaseReview from '../components/PurchaseReview.jsx';
import PurchaseAnalysis from '../components/PurchaseAnalysis.jsx';
...
export function purchaseSaveToast({ matching = false, removing = false, editing = false, status, previewMonth, onOpenAlerts }) {
  ...
  if (status?.tone === 'warn' || status?.tone === 'bad') return {
    title: 'Purchase saved — plan needs attention',
    body: 'The purchase is saved, but the calculated shortfall is not resolved. Reopen it to adjust the cost or date.',
    tone: status.tone,
    actions: [],
  };
...
export default function PurchaseDrawer({ id, initialDate, base, baseVersion, plan, refresh, onOpenAlerts, onClose }) {
  ...
  const save = async (action = 'save') => {
    ...
    try {
      ...
      setSaved({ matching, removing });
      if (await refresh()) {
        toast.push(purchaseSaveToast({ matching, removing, editing: Boolean(existing), status, previewMonth, onOpenAlerts }));
        onClose();
      } else setError('Your purchase was ' + (matching ? 'matched' : removing ? 'removed' : 'saved') + ', but the updated plan could not reload. Reload purchases to see the change.');
  ...
  const reload = async () => {
    ...
    setBusy(true);
    try {
      if (await refresh()) { setError(''); if (saved) onClose(); }
      else setError('The updated plan still could not load. Please try reloading again.');
  ...
  return <Drawer label={title} onClose={close} className="purchase-drawer" protectChanges={!saved} busy={busy}><DrawerHeader title={title} icon="cart" onClose={close} />
    ...
    {preview?.remove && <section className="purchase-removal" aria-label="Remove purchase confirmation">
      <h3>Remove {existing.label}?</h3>
      <p>{budgetMoney(existing.amount)} · {budgetDate(existing.date)}</p>
      <p>This removes the purchase from your plan. It does not cancel an order or delete a bank transaction.</p>
      <details><summary>See the budget change</summary><BudgetImpact impact={preview.impact} /></details>
      <div className="row wrap budget-actions"><button className="btn" disabled={busy || !!saved} onClick={() => save()}>{busy ? 'Removing…' : 'Remove purchase'}</button>
    ...
    {matches && <section aria-label="Match a posted charge"><h3>Is one of these your purchase?</h3>
      ...
      <div className="row wrap budget-actions"><button className="btn" disabled={!selected || busy || !!saved} onClick={() => save('match')}>{busy ? 'Confirming…' : 'Confirm this is the purchase'}</button>
        <button className="btn ghost" disabled={busy || !!saved} onClick={() => setMatches(null)}>Back to details</button></div>
    </section>}
    {busy && <p role="status">Checking and saving your plans…</p>}
    {error && <div className="alert" role="alert"><p>{error}</p><button className="btn ghost sm" onClick={reload} disabled={busy}>Reload purchases</button></div>}
  </Drawer>;
}
