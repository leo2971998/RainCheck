...
function SavingsEditor({ base, baseVersion, plan, protectedIds = {}, result, change, onClose }) {
  ...
  // Protection constrains the optimizer; an explicit edit overrides it for this proposal only.
  const previewProtection = Object.fromEntries(Object.entries(protectedIds).filter(([id]) => !manualTargets[id]));
  ...
  const signature = JSON.stringify([baseVersion || base, plan, protectedIds, draft, manualTargets]);
  const preview = reviewed?.signature === signature ? reviewed.value : null;
  const edit = (field, id, value) => { if (field === 'targets') setManualTargets(ids => ({ ...ids, [id]: true })); setDraft(d => ({ ...d, [field]: { ...d[field], [id]: value } })); setReviewed(null); setError(''); };
  const calculate = e => {
    e.preventDefault(); setError('');
    try { setReviewed({ signature, value: savingsPreview(base, plan, draft, previewProtection) }); }
  ...
  return <>
    ...
    <form className="savings-form" onSubmit={calculate}>
      <section><h3>1. Choose monthly spending targets</h3><p className="fine">Edit any target below, then preview your changes. Keep unchanged applies to the optimizer’s suggestions; you can still adjust those categories manually. Keep replacement costs in mind—for example, eating at home can increase groceries.</p>
        <div className="savings-fields">{insights.categories.map(c => <div className="savings-field" key={c.id}>
          <div><label htmlFor={`saving-${c.id}`}><b>{c.label}</b><small>Current budget {money(c.budget)}{c.protected ? (manualTargets[c.id] ? ' · manually adjusted' : ' · kept unchanged by optimizer') : ''}</small></label>
            </div>
