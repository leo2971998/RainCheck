...
if (!process.argv.includes('--apply')) {
  ...
  try {
    ...
    const migrations = [];
    for (const file of readdirSync(new URL('../supabase/migrations/', import.meta.url)).filter(f => /^\d{12}_.+\.sql$/.test(f)).sort()) {
      ...
      migrations.push({ version, status: previous ? 'already applied' : 'applied' });
    }
    const verification = await verifyDatabase(client, { requireData: false });
    await client.query('commit');
    console.log(JSON.stringify({ migrations, ...verification }, null, 2));
  } catch (error) {
